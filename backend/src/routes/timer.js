...
router.post('/sessions', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO timer_sessions (id,assignment_id,started_at,ended_at,duration_minutes,session_type)
    VALUES (?,?,?,?,?,?)
  `).run(id, assignment_id || null, started_at, ended_at || null, duration_minutes || null, session_type)
  ...
  if (assignment_id && duration_minutes) {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(hours, Math.floor(Date.now() / 1000), assignment_id)
  }
...
router.get('/stats', (req, res) => {
  ...
  const stats = db.prepare(`
    ...
    FROM timer_sessions
    WHERE started_at >= ? AND session_type = 'focus'
  `).get(weekStart)
  res.json(stats)
})
